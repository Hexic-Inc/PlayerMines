...
public class CommandManager implements TabExecutor {
    ...
    public CommandManager() {
        ...
        //Admin commands
        adminCommands.add(new tp());
        adminCommands.add(new delete());
        adminCommands.add(new SpawnMC());
    }
            ...
            if(args.length > 1 && args[0].toLowerCase().contains("admin")){
                for(SubCommand subCommand : adminCommands){
                    if(args[1].equalsIgnoreCase(subCommand.getName())){
                        if(Main.getInitalizer().getPerms().playerHas(p, subCommand.getPermission()) || p.isOp()){
