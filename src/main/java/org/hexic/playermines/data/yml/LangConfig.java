...
import org.hexic.playermines.handlers.PlaceHolderHandler;
...
public class LangConfig {
    ...
    public LangConfig(Player player){
        this.placeHolderHandler = new PlaceHolderHandler(player);
        config = PlayerMines.getInitalizer().getDataManager().getConfig("Lang.yml");
    }

    public LangConfig(Player player, Upgrade upgrade){
        this.placeHolderHandler = new PlaceHolderHandler(player, upgrade);
        config = PlayerMines.getInitalizer().getDataManager().getConfig("Lang.yml");
    }
    public LangConfig(){
        config = PlayerMines.getInitalizer().getDataManager().getConfig("Lang.yml");
    ...
    public String getValue(String section, String key, String defaultValue){
        if(config.get(section) == null){
            ...
            config.saveConfig();
            return translateToColorCode(placeHolderHandler.replace( defaultValue));
        }
        ...
        return translateToColorCode(placeHolderHandler.replace( config.getString(section + "." + key)));
    }
    ...
    public String getPrefixValue(String section, String key, String defaultValue){
        if(config.get(section) == null){
            ...
            config.setListValue(section, key, defaultValue);
            config.saveConfig();
            return getPrefix() +translateToColorCode(placeHolderHandler.replace( defaultValue));
        }
        if(config.getString(section + "." + key) == null){
            config.setListValue(section, key, defaultValue);
            config.saveConfig();
        ...
        return getPrefix() + translateToColorCode(placeHolderHandler.replace( config.getString(section + "." + key)));
    }
